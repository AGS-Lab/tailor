...
class VaultNotFoundError(VaultError):
    ...
    def __init__(self, vault_path: str):
        super().__init__(
            f"Vault directory not found: {vault_path}", {"vault_path": vault_path}
        )
...
class JSONRPCError(WebSocketError):
    ...
    def __init__(self, message: str, code: int = -32603):
        super().__init__(f"JSON-RPC error: {message}", {"code": code})
        self.code = code
...
class CommandNotFoundError(CommandError):
    ...
    def __init__(self, command_id: str, available_commands: Optional[list] = None):
        ...
        super().__init__(f"Command '{command_id}' not found", details)
...
class MissingConfigurationError(ConfigurationError):
    ...
    def __init__(self, key: str):
        super().__init__(f"Required configuration '{key}' is missing", {"key": key})
...
class InvalidPathError(PathError):
    ...
    def __init__(self, path: str, reason: str):
        super().__init__(
            f"Invalid path '{path}': {reason}", {"path": path, "reason": reason}
        )
...
class PathTraversalError(PathError):
    ...
    def __init__(self, path: str):
        super().__init__(f"Path traversal attempt detected: {path}", {"path": path})
