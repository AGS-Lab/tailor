...
class TestWebSocketServer:
    ...
    async def test_handle_message_valid_request(self, server):
        ...
        with patch.dict(
            "sys.modules",
            {
                "sidecar.vault_brain": MagicMock(
                    VaultBrain=MagicMock(get=MagicMock(return_value=mock_brain))
                )
            },
        ):
    ...
    async def test_handle_message_method_not_found(self, server):
        ...
        mock_brain.execute_command = AsyncMock(
            side_effect=exceptions.CommandNotFoundError("unknown", [])
        )
    ...
    async def test_handler_exception(self, server):
        ...
        with patch.dict(
            ...
            assert (
                "Handler failed" in response["error"]["message"]
                or "ValueError" in response["error"]["message"]
            )
