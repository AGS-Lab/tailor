...
class DefaultPipeline:
    ...
    async def complete(
        ...
        **kwargs,
        ...
        if not self.llm_service:
            ...
            return LLMResponse(
                content="[Error] LLM service not initialized", model="none"
            )

        return await self.llm_service.complete(
            messages=messages, category=category, stream=stream, **kwargs
        )
    ...
    async def run(
        ...
        initial_state = PipelineContext(
            message=message, original_message=message, history=history or []
        )
