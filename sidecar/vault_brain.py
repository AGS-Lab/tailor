...
class VaultBrain:
    ...
    async def initialize(self) -> None:
        ...
        pipeline_config = (
            PipelineConfig(**llm_config) if llm_config else PipelineConfig()
        )
        ...
        for plugin_dir in plugin_dirs:
            ...
            if settings_path.exists():
                try:
                    ...
                    logger.error(
                        f"Failed to load settings.json for plugin '{plugin_name}': {e}"
                    )
            ...
            try:
                ...
                if not spec or not spec.loader:
                    raise exceptions.PluginLoadError(
                        plugin_name, "Failed to create module spec"
                    )
                ...
                if not hasattr(module, constants.PLUGIN_CLASS_NAME):
                    raise exceptions.PluginLoadError(
                        plugin_name, f"No '{constants.PLUGIN_CLASS_NAME}' class found"
                    )
    ...
    async def _activate_plugins(self):
        ...
        for plugin_name, plugin in self.plugins.items():
            try:
                ...
                await self.publish(
                    constants.CoreEvents.PLUGIN_LOADED, plugin_name=plugin_name
                )
            except Exception as e:
    ...
    def register_command(
        ...
        if not inspect.iscoroutinefunction(handler):
            raise exceptions.CommandRegistrationError(
                command_id, "Handler must be an async function"
            )
    ...
    async def get_info(self) -> Dict[str, Any]:
        return {"vault": self.config.get("name"), "plugins": list(self.plugins.keys())}
    ...
    async def list_providers(self, **kwargs) -> Dict[str, Any]:
        ...
        return {"status": "success", "providers": self._keyring.get_provider_status()}
    ...
    async def set_model_category(
        ...
        try:
            ...
            return {"status": "success", "category": category, "model": model}
        except Exception as e:
    ...
    async def chat_set_model(
        self, chat_id: str = "", model_id: str = "", category: str = "", **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        if not model_id and not category:
            return {
                "status": "error",
                "error": "Either model_id or category is required",
            }
        ...
        try:
            ...
            selected_model = (
                model_id
                if model_id
                else self._llm_service.get_model_for_category(category)
            )
    ...
    async def chat_send(
        ...
        **kwargs,
        ...
        if chat_id:
            try:
                metadata_res = await self.execute_command(
                    "chat.get_metadata", chat_id=chat_id, key="model_override"
                )
    ...
    async def install_plugin(
        ...
        if download_url:
            result = await self.plugin_installer.install_from_url(
                download_url, plugin_id
            )
        elif repo_url:
    ...
    def notify_frontend(
        self, message: str, severity: str = constants.Severity.INFO
    ) -> None:
        """Send a notification toast to the Frontend."""
        self.emit_to_frontend(
            constants.EventType.NOTIFY, {"message": message, "severity": severity}
        )

    def update_state(self, key: str, value: Any) -> None:
        """Update a key in the Frontend global/vault state."""
        self.emit_to_frontend(
            constants.EventType.UPDATE_STATE, {"key": key, "value": value}
        )
